...
| `settingsStore: SettingsStore` | Key-value registry of user-configurable settings with type validation | `_settings: Map<string, RegisteredSetting>` | `registerSetting(owner, name, meta)`, `getStringSetting(id)`, `setSetting(id, value)`, `resetSetting(id)` | (none) | All extensions, BasemapStore (basemap setting), LayerTreeStore (data source URL) |
...
| `mapStore: MapStore` | MapLibre GL map instance lifecycle, deck.gl overlay, low-level style manipulation | `map`, `overlayManager`, `layerManager` | `initializeMap(container)`, `setMap(map)`, `zoomToExtent(bbox)`, `context` (computed), `applyStyleFragment(style, prefix, ownedIds)`, `removeOwnedLayers(ownedIds)` | `mapToolStore` | Widgets, map tools, rendering pipeline, BasemapStore |
| `basemapStore: BasemapStore` | Basemap config registry, cached health checks, basemap application, reactive sync | `_configs: BaseMapConfig[]`, `_healthCache`, `_ownedIds`, `_initInProgress` | `registerConfigs(configs)`, `checkHealth(id, timeout?, force?)`, `applyBasemap(config)`, `startSync()`, `stopSync()`, `findAndApplyInitialBasemap()`, `availableBasemaps`, `activeBasemap`, `setActiveBasemap(id)` | `settingsStore`, `mapStore` (low-level style ops) | BasemapTool (Grid UI), map lifecycle hook |
