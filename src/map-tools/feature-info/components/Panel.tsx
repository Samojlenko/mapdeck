...
export const FeatureInfoComponent: (
...
) => React.ReactNode = observer(({ map, rootStore, deactivate }) => {
    ...
    const getVisibleLayers = React.useCallback((): LayerNode[] => {
        return rootStore.treeStore.layerNodes.filter((node) => node.isVisible);
    }, [rootStore.treeStore]);
            ...
            featureCollector
                ...
                .catch((error) => {
                    if (!controller.signal.aborted) {
                        logger.error("FeatureInfo: collection failed", error);
                        setLoading(false);
                    }
                });
        },
        [map, getVisibleLayers, dict],
    );
    ...
    }, [map, handleMapClick, handleKeyDown]);
    ...
    return (
        ...
        >
            {!hasClicked && (
                <div className={styles.noFeatures}>{dict["noClickYet"]}</div>
            )}

            {hasClicked && groups.length === 0 && !loading && (
                <div className={styles.noFeatures}>{dict["noFeatures"]}</div>
            )}
            ...
            <div className={styles.layerSelector}>
                ...
                >
                    {groups.length === 0 && (
                        <option value={0}>{dict["select.placeholder"]}</option>
                    )}
